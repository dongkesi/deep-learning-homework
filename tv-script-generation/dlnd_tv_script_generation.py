...
def create_lookup_tables(text):
    ...
    vocab_to_int = {word[0]:i for i, word in enumerate(words)}
    int_to_vocat = {i:word for word, i in vocab_to_int.items()}
    return vocab_to_int, int_to_vocat
...
def build_nn(cell, rnn_size, input_data, vocab_size, embed_dim):
    ...
    logits = tf.contrib.layers.fully_connected(output, vocab_size, activation_fn=None,                                               weights_initializer=tf.truncated_normal_initializer())
...
def get_batches(int_text, batch_size, seq_length):
    ...
    text = int_text[:n_batches*n_chars]
    
    x = np.array(int_text[:n_batches*n_chars])
    y = np.array(int_text[1:n_batches*n_chars])
    y = np.append(y, ([x[0] if len(int_text) == n_batches*n_chars else int_text[n_batches*n_chars]]))
